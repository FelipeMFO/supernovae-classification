...
\hline
  ...
  \label{chap:GP}
	...
	Antes adentrar nas escolhas das \textit{Kernel Functions} é muito importante explicar o fator da aleatoriedade dentro do projeto. Originalmente a biblioteca PyMC3 realiza métodos de otimização da função a posteriori em seu GP, entretanto por ser uma biblioteca que exige um alto custo computacional, dentro desse projeto decidiu-se optar por melhor escolher uma função Kernel para a interpolação em prol de não executar a linha de código que otimizaria os parâmetros da distribuição a posteriori (ilustrado como as distribuições a direita na figura \ref{fig:MCMC1}). 

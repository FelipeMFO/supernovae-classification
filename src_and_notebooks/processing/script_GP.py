...
def GP_prep(file, filters, clean_method = '', gp_method = 'm52'):
...
def GP_matern32(t,x,y,yerr):
    ...
    with pm.Model() as model:
        ...
        cov = eta * pm.gp.cov.Matern32(1, lengthscale)        
...
def GP_ratquad(t,x,y,yerr):
    ...
    with pm.Model() as model:
        ...
        cov = eta * pm.gp.cov.RatQuad(1, lengthscale, alfa)
...
def GP_expquad(t,x,y,yerr):
    ...
    with pm.Model() as model:
        eta = pm.HalfCauchy("N", beta=2, testval=2.0)
        lengthscale = pm.Gamma("L", alpha=4, beta=0.1)
        cov = eta * pm.gp.cov.ExpQuad(1, lengthscale)      
